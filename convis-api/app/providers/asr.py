...
class GoogleASR(ASRProvider):
    ...
    def get_cost_per_minute(self) -> float:
        """Cost: $0.006/min (Google)"""
        return 0.006


class WhisperLocalASR(ASRProvider):
    """
    Local faster-whisper ASR Provider (via faster-whisper-server)

    Latency: 100-300ms (depends on hardware)
    Cost: $0.00/min (self-hosted)
    Best for: Privacy, no API costs, offline usage
    """

    def __init__(self, api_key: Optional[str] = None, model: str = "medium", language: str = "en", **kwargs):
        super().__init__(
            api_key=api_key or os.getenv("WHISPER_API_URL", "http://localhost:8080"),
            model=model,
            language=language
        )
        self.base_url = self.api_key
        if not self.base_url.endswith("/v1/audio/transcriptions"):
            self.transcription_url = self.base_url.rstrip("/") + "/v1/audio/transcriptions"
        else:
            self.transcription_url = self.base_url
        self.logger.info(f"WhisperLocal ASR initialized with URL: {self.transcription_url}, model: {self.model}")

    async def transcribe_stream(self, audio_stream):
        """Streaming not natively supported - buffer and transcribe"""
        import aiohttp
        buffer = b''
        async for chunk in audio_stream:
            buffer += chunk
    ...
    async def transcribe(self, audio_bytes: bytes) -> str:
        """Transcribe audio by sending to local faster-whisper server"""
        import aiohttp
        import io
        import wave
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(self.transcription_url, data=form_data, timeout=aiohttp.ClientTimeout(total=10)) as response:
                    if response.status == 200:
                        result = await response.json()
                        return result.get('text', '').strip()
                    else:
                        error = await response.text()
                        self.logger.error(f"WhisperLocal transcription failed: {response.status} - {error}")
                        return ""
        except Exception as e:
            self.logger.error(f"WhisperLocal transcription error: {e}")
            return ""
